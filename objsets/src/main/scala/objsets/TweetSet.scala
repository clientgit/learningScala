...
abstract class TweetSet extends TweetSetInterface:
  ...
  def mostRetweetedCurrent(biggest: Tweet): Tweet
...
class Empty extends TweetSet:
  ...
  def mostRetweetedCurrent(biggest: Tweet): Tweet = biggest
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = mostRetweetedCurrent(elem)

  def mostRetweetedCurrent(elem: Tweet): Tweet =
    val biggest = right.mostRetweetedCurrent(left.mostRetweetedCurrent(elem))
    if(biggest.retweets > elem.retweets) biggest else elem
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = allTweets.filter((tw: Tweet) => google.exists(keyWord => tw.text.contains(keyWord)))
  lazy val appleTweets: TweetSet = allTweets.filter((tw: Tweet) => apple.exists(keyWord => tw.text.contains(keyWord)))
